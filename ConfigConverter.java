 ...
 *
 * <p>Поддерживаемая грамматика соответствует условию задания:
 * <ul>
 *   <li>Однострочные комментарии начинаются с {@code REM} и продолжаются до конца строки.</li>
 *   <li>Многострочные комментарии начинаются с {@code (comment} и заканчиваются символом {@code )}.</li>
 *   <li>Числа — десятичные значения с необязательным знаком и дробной частью
 *       (например {@code 3.14}, {@code -0.5}). Внутри константных выражений также допускаются целые.</li>
 *   <li>Строки начинаются с символа {@code @}, затем идёт текст в двойных кавычках.
 *       Экранирование не обрабатывается (символы берутся как есть).</li>
 *   <li>Массивы: в квадратных скобках, элементы разделены точкой с запятой:
 *       {@code [ 1.0; 2.0; 3.0 ]}.</li>
 *   <li>Словари: в фигурных скобках, пары разделены запятыми:
 *       {@code { key : value, other : value } }. Ключи должны соответствовать {@code [a-z]+}.</li>
 *   <li>Объявление констант: {@code name := value}. Значение доступно далее по тексту.</li>
 *   <li>Константные выражения: заключаются в вертикальные черты и задаются в постфиксной форме (ОПН).
 *       Поддерживаются {@code +}, {@code -}, {@code *}, {@code concat()} и {@code mod()}.</li>
 * </ul>
 *
 * <p>Результирующая структура (последнее встретившееся НЕ-объявление) сериализуется в YAML.
